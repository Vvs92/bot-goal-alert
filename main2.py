...
LEAGUES = {2: "Champions League", 5: "Europa League", 8: "Premier League", 564: "La Liga", 384: "Serie A", 82: "Bundesliga", 301: "Ligue 1", 72: "Bundesliga 2", 48: "Championship", 37: "Super Lig", 377: "Belgian Pro League", 955: "Saudi Pro League"}
...
def get_fixtures():
    try:
        r = requests.get(URL + "/livescores/inplay", headers=HEAD, params={"include": "participants;scores;state;statistics", "per_page": 50}, timeout=15)
    ...
    try:
        for s in fixture.get("scores", []):
            if s.get("description") == "CURRENT":
                sd = s.get("score", {})
                return sd.get("goals", 0) or 0 if home else sd.get("participant", 0) or 0
    except:
        pass
    return 0
...
async def send_alert(bot, fixture, score, info):
    ...
    lvl = "ALERTE MAX" if score >= 85 else "FORTE PRESSION" if score >= 75 else "PRESSION"
    msg = "BUT POTENTIEL - " + lvl + "\n\n" + league + "\n" + h + " " + str(hg) + " - " + str(ag) + " " + a + "\nMin: " + str(minute) + "'\nScore: " + str(score) + "/100\n" + gauge + "\n\n" + "\n".join(["- " + i for i in info])
    try:
...
**7.** Commit → puis va modifier le **`Dockerfile`** et remplace `main.py` par `main2.py` :
```
CMD ["python", "main2.py"]
