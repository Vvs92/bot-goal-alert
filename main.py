...
def run_diagnostic():
    """
    Teste tous les endpoints Bzzoiro et retourne un rapport complet.
    Permet de voir exactement quels IDs de ligues sont utilises.
    """
    lines = []
    ...
    d = api_get("/api/live/")
    if d is None:
        lines.append("X /api/live/ : ECHEC (cle invalide?)")
    else:
        n = len(d.get("results", []))
        lines.append("OK /api/live/ : " + str(n) + " matchs")
        for m in d.get("results", [])[:3]:
            lg  = m.get("league", {})
            lid = lg.get("id", "?") if isinstance(lg, dict) else "?"
            lnm = lg.get("name", "?") if isinstance(lg, dict) else "?"
            lines.append("   id=" + str(lid) + " [" + str(lnm) + "] "
                         + str(m.get("home_team","?"))[:10]
                         + " vs " + str(m.get("away_team","?"))[:10]
                         + " st=" + str(m.get("status","?")))

    # Test /api/events/?status=inprogress
    d2 = api_get("/api/events/", {"status": "inprogress"})
    if d2 is None:
        lines.append("X /api/events/?status=inprogress : ECHEC")
    else:
        n2 = len(d2.get("results", []))
        lines.append("OK /api/events/inprogress : " + str(n2) + " matchs")
        for m in d2.get("results", [])[:5]:
            lg  = m.get("league", {})
            lid = lg.get("id", "?") if isinstance(lg, dict) else "?"
            lnm = lg.get("name", "?") if isinstance(lg, dict) else "?"
            lines.append("   id=" + str(lid) + " [" + str(lnm) + "] "
    ...
    d3 = api_get("/api/events/", {"status": "1st_half"})
    if d3:
        n3 = len(d3.get("results", []))
        lines.append("OK /api/events/1st_half : " + str(n3) + " matchs")

    # Test /api/events/?status=2nd_half
    d4 = api_get("/api/events/", {"status": "2nd_half"})
    if d4:
        n4 = len(d4.get("results", []))
        lines.append("OK /api/events/2nd_half : " + str(n4) + " matchs")

    # Ligues disponibles
    dl = api_get("/api/leagues/")
    if dl:
        leagues = dl.get("results", [])
        lines.append("Ligues dispo: " + str(len(leagues)))
        for lg in leagues:
...
# ================================================================
# RECUPERATION MATCHS LIVE
# ================================================================

def get_live_matches():
    """
    Essaie /api/live/ puis fallback /api/events/ avec statuts live.
    Filtre sur WATCHED_LEAGUES + LIVE_STATUSES.
    """
    key = "live"
...
def build_message(match, m, pred, threshold):
    ...
    if pred:
        ...
        if m["p_over25"] > 0:
            ic = "\U0001f7e2" if m["p_over25"] >= 65 else ("\U0001f7e1" if m["p_over25"] >= 45 else "\U0001f534")
        ...
        if m["p_btts"] > 0:
            ic = "\U0001f7e2" if m["p_btts"] >= 60 else ("\U0001f7e1" if m["p_btts"] >= 45 else "\U0001f534")
    ...
    if dom_name:
        if dom_eff == "efficace":
            recs.append("  \u2192 \u26bd Prochain but: " + dom_name + " (dom. + efficace) \U0001f3af")
