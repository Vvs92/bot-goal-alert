...
def get_live_matches():
    """
    GET /api/live/
    Filtre sur statuts live exacts (doc officielle) et nos ligues.
    Log TOUS les matchs recus pour diagnostic.
    """
    key = "live"
    ...
    if not data:
        live_cache[key] = (time.time(), [])
        return []

    results = data.get("results", [])
    total   = len(results)

    # Log TOUS les matchs pour diagnostic complet
    print("  [LIVE] " + str(total) + " matchs recus:", flush=True)
    for m in results:
        lg  = m.get("league", {})
    ...
    print("  [LIVE] " + str(len(filtered)) + "/" + str(total)
          + " dans nos ligues (live)", flush=True)
    return filtered
