...
“””
GET /api/live/
Cache 30s. Log TOUS les matchs pour diagnostic des IDs de ligues.
“””
key = “all”
...
# LOG TOUS LES MATCHS LIVE pour voir les vrais IDs de ligues
if total == 0:
    print("  [LIVE] Aucun match live retourne par l'API", flush=True)
else:
    ...
    for m in results:
        ...
        s = str(m.get("status", "?"))
        print("    league_id=" + str(lid) + " [" + lname + "] "
              + h[:10] + " vs " + a[:10] + " statut=" + s, flush=True)
        ...
        if not matches:
            ...
            for match in matches:
                try:
                    ...
                    # DEBUG: log TOUS les matchs recus pour voir les vrais statuts
                    print("    [M] " + h_name[:11] + " vs " + a_name[:11]
                          + " statut='" + status + "'"
                          + " min=" + str(minute)
                          + " " + str(hg) + "-" + str(ag), flush=True)

                    # Ignorer uniquement matchs clairement termines/pas commences
                    SKIP = {"notstarted","not started","ns","finished",
                            "ft","aet","pen","postponed","cancelled",
                            "abandoned","suspended"}
                    if status in SKIP:
                        continue
                    # Ignorer mi-temps
                    if status in ("ht", "halftime", "half time", "half-time"):
                        continue
                    # Si minute=0 et statut inconnu, on laisse passer quand meme
                    # pour voir dans les logs ce qui se passe
