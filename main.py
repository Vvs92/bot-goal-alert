...
period (1T, HT, 2T, FT)
status (notstarted|inprogress|1st_half|halftime|2nd_half|finished|po
...
top_keys = list(data.keys()) if isinstance(data, dict) else ["(liste " + str(len(data
...
short = (parts[0][0] + ". " + " ".join(parts[1:])) if len(parts) > 1 else playe
...
elif minute < 12: elif minute < 22: elif minute < 32: base += 12
base += 7
base += 3
diff = abs(hg - ag)
if diff == 0:
